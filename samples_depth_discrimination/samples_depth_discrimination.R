...
#save(all_deapths, file="mit.all.deapths.RData")
...
g <- ggplot(dfm, aes(x = base_pair, y = coverage, colour = sample)) + geom_line() + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) + geom_vline(xintercept = min)
...
amplicon2 <- all_deapths[623:8982,]
...
names(which(colMeans(amplicon1) < threshold))
names(which(colMeans(amplicon2) < threshold))
...
fag <- ggplot(filt_a, aes(x = base_pair, y = coverage, colour = sample)) + ggtitle(paste(c("amplicon 1, filter <= ", threshold), collapse = " ")) + geom_line() + theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank()) + geom_vline(xintercept = 16570)  + ylim(0, 4000)
fbg <- ggplot(filt_b, aes(x = base_pair, y = coverage, colour = sample)) + ggtitle(paste(c("amplicon 2, filter <= ", threshold), collapse = " ")) + geom_line()+ theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank())  + ylim(0, 4000)
